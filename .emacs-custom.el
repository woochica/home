(custom-set-variables
  ;; custom-set-variables was added by Custom.
  ;; If you edit it by hand, you could mess it up, so be careful.
  ;; Your init file should contain only one such instance.
  ;; If there is more than one, they won't work right.
 '(auto-compression-mode t nil (jka-compr))
 '(auto-fill-mode t)
 '(browse-url-browser-function (quote browse-url-generic))
 '(browse-url-generic-program "/usr/bin/firefox")
 '(calendar-week-start-day 1)
 '(case-fold-search t)
 '(current-language-environment "UTF-8")
 '(default-input-method "rfc1345")
 '(diary-file "~/slink/diary")
 '(display-time-default-load-average nil)
 '(display-time-format "%y.%m.%d. %H.%M")
 '(display-time-mode nil)
 '(ecb-options-version "2.33beta2")
 '(echo-keystrokes 0.2)
 '(emms-lastfm-username "nyuhuhuu")
 '(erc-nick "nyuhuhuu")
 '(erc-server "im.bitlbee.org")
 '(erc-user-full-name "Török Gábor")
 '(fill-column 79)
 '(fringe-mode (quote (0)) nil (fringe))
 '(git-show-unknown nil)
 '(global-font-lock-mode t nil (font-lock))
 '(global-hl-line-mode t)
 '(indicate-buffer-boundaries nil)
 '(indicate-empty-lines t)
 '(js2-basic-offset 4)
 '(kill-whole-line t)
 '(menu-bar-mode t)
 '(mouse-yank-at-point t)
 '(org-agenda-files (quote ("~/slink/webma.org")))
 '(org-default-notes-file "~/slink/slink.org")
 '(org-directory "~/slink")
 '(rhino-rhino-jar-path "/home/gabor/src/yuicompressor-2.2.4/lib/rhino-1.6R7.jar")
 '(scroll-bar-mode nil)
 '(show-paren-mode t nil (paren))
 '(show-paren-style (quote mixed))
 '(show-trailing-whitespace t)
 '(tool-bar-mode nil)
 '(tooltip-mode nil)
 '(twit-show-user-images t)
 '(twit-user "nyuhuhuu")
 '(weblogger-config-alist (quote (("20y" ("user" . "admin") ("server-url" . "http://20y.hu/xmlrpc.php") ("weblog" . "1")) ("autoszektor-comments" ("user" . "webma.autoszekt@gmail.com") ("server-url" . "http://www2.blogger.com/api") ("weblog" . "1411866265685396192")))))
 '(webma-css-rhino-path "/home/gabor/dev/yuicompressor-2.2.5/build/yuicompressor-2.2.5.jar")
 '(webma-instance-idb-tool "~/mnt/i2/tools/webma_search/tools/makeidb.sh")
 '(webma-siteanalyze-path "/home/gabor/dev/webma/webmastertools/siteanalyze.py"))

(custom-set-faces
  ;; custom-set-faces was added by Custom.
  ;; If you edit it by hand, you could mess it up, so be careful.
  ;; Your init file should contain only one such instance.
  ;; If there is more than one, they won't work right.
 '(css-property ((t (:inherit font-lock-variable-name-face :weight bold))))
 '(css-selector ((t (:inherit font-lock-function-name-face :weight bold))))
 '(flymake-errline ((t (:background "black" :underline "red4" :weight bold))))
 '(font-lock-comment-face ((t (:foreground "IndianRed" :slant normal :weight normal :height 1.0 :family "Andale Mono"))))
 '(font-lock-warning-face ((t (:inherit font-lock-warning :background "#aa0000" :foreground "#ffffff"))))
 '(mumamo-background-chunk-submode ((((class color) (min-colors 88) (background dark)) (:background "gray15"))))
 '(show-paren-match ((t (:bold t :background "#eeca00" :foreground "#763b17"))))
 '(twit-message-face ((default nil) (nil nil)))
 '(twit-title-face ((t (:underline "DeepSkyBlue"))))
 '(twit-zebra-2-face ((((class color) (background dark)) nil)))
 '(which-func ((((class color) (min-colors 88) (background dark)) (:foreground "Pink")))))
